	...
	while (temp_Node != NULL)
	{
		if (insert_data > temp_Node->data) 
		{
			if (temp_Node->Right_child == NULL)
			{
				...
				temp_Node->Right_child->Father_child = temp_Node;
		...
		else
		{
			if (temp_Node->Left_child == NULL)
			{
				...
				temp_Node->Left_child->Father_child = temp_Node;
				if (temp_Node->color_tag == 1)
				{
...
{
	RB_Tree_Node<T>* temp_Node = Root_Node;
	while (temp_Node->data != delete_data && temmp != NULL)
	{
	...
	//满足上面的循环，那么temp_Node符合要求，并且存在，下面的循环直接利用上面循环的结果
	if (temp_Node)
	{
			...
			//左子树不为空,右子树为空
		{
			...
			//左右子树都不为空
		{
			RB_Tree_Node<T>* Successor_Node = Finde_Successor_Node(temp_Node);
...
void RB_Tree<T>::Fix_Tree(RB_Tree_Node<T>* current_Node)
{
	...
	while (true)
	{ 
		//只有插入的当前节点不是根节点 并且 他的父节点是红色 的时候才需要修正
		...
		if (grandfa_Node)
		{
			if (father_Node == grandfa_Node->Left_child)
			{
				...
				if (uncle_Node)
				{
					...
					//情况3：叔叔是黑色，且当前节点为左孩子 将父节点设为黑色 祖父设为红色 对祖父节点右旋
					else
					{
						father_Node->color_tag = 0;
						grandfa_Node->color_tag = 1;
			...
			else
			{
				...
				if (uncle_Node)
				{
					...
					//叔叔是黑色，且当前节点为右孩子 将父节点设为黑色 祖父设为红色 对祖父节点左旋
...
void RB_Tree<T>::Right_Rotate(RB_Tree_Node<T>* current_Node)
{
	...
	{
		father_Node->Right_Node = left_Node;
	}
...
******************************************************************/
template<class T>
RB_Tree_Node<T>* RB_Tree<T>::Finde_Successor_Node(RB_Tree_Node<T>* current_Node)
{
